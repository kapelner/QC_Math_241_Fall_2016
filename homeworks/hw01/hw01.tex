...
\title{MATH 241 Spring 2016 Homework \#1}
...
\date{Due 5PM outside my office KY604, Tuesday, Sep. 6, 2015 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem If $\braces{A,B,C}$ are collectively exhaustive, simplify $A \cup B \cup C$. \spc{1}

\intermediatesubproblem If $\braces{A,B,C}$ are collectively exhaustive, simplify $A \cap B \cap C$. \spc{2}

\intermediatesubproblem If $\braces{A,B,C}$ are mutually exclusive, simplify $A \cup B \cup C$. \spc{2}

\easysubproblem If $\braces{A,B,C}$ are mutually exclusive, simplify $A \cap B \cap C$. \spc{1}
...
\begin{enumerate}
\easysubproblem Draw this outcome space in a Venn Diagram. Use a rectangle for $\Omega$.  \spc{3}

\easysubproblem What is $\abss{\Omega}$?  \spc{1}

\intermediatesubproblem What is the size of the event space? \spc{2}
...
\easysubproblem Using our \qu{working definition} of probability, what is the probability of heads or an even number? \spc{3}
...
\easysubproblem Draw a number line for $Z \subset \reals$ where $Z := \braces{x \in \reals : |x| > 2}$. This $Z$ notation we'll be using in a couple months when we get to the normal distribution. $Z$ is not $\integers$. \spc{2}
...
\intermediatesubproblem How many ways are there to set up the pieces in the back ranks of both white and black \ie arrange the two rooks, two knights, two bishops, king and queen on the first row of 8 squares. Note that this game is called ``Fischer Random Chess'' after the famous grandmaster Bobby Fischer who proposed the idea to make standard chess more exciting.  \spc{3}

\hardsubproblem The game progresses and white takes two black pawns and black takes two white pawns. How many ways are there to arrange the pieces on the board? We don't care about pieces of a type being unique (\ie all white pawns are the same, all black rooks are the same, etc).  \spc{5}

\hardsubproblem In the answer to (c) are all arrangements \qu{equally likely} during an actual chess game? Explain why or why not. \spc{3}
